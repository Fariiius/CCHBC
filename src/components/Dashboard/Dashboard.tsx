...
export const Dashboard = () => {
  const {
    loading, error, tables, stagingFile, clearStaging,
  } = useDashboard();

  if (loading && tables.length === 0 && !stagingFile) {
    return (
  ...
  if (error && tables.length === 0 && !stagingFile) {
    return (
      <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'var(--background)' }}>
        <div style={{ background: '#fce8e6', border: '1px solid #f5c6cb', borderRadius: 8, padding: '1.5rem 2rem', maxWidth: 400, textAlign: 'center' }}>
          ...
          <button onClick={clearStaging} style={{ padding: '0.5rem 1.25rem', borderRadius: 6, background: '#d93025', color: 'white', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer', border: 'none' }}>Try Again</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
         {stagingFile ? <StudioView /> : (tables.length > 0 ? <DashboardView /> : <UploadZone />)}
      </div>
    </div>
  );
};
